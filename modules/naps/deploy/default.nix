{flakeRoot, lib, inputs, pkgs, config,...}:

let

    utils = import "${flakeRoot}/lib" {inherit lib inputs;};


    processVM =
        vmname: {ip, services, containers, ...}:
        let
            mkEnv = name: utils.mkCTEnv vmname name;
            ctconfs = lib.imap 
                        (i: name:
                        let env = mkEnv name;
                        in {
                            ${utils.envUID env} = {
                                ip = "192.168.100.${lib.toString (50+i)}";
                                env = mkEnv name;
                            };
                            ${utils.envHost env}.containers = [env];
                        })
                        containers;
            vmconf = {
                        ${vmname} = {
                            env = utils.mkVMEnv vmname;
                            inherit ip;
                        };
                     };
        in utils.mergeAll ([vmconf]  ++ ctconfs);


    processService =  #generates the endpoints list
        _: {endpoints, deployements,...}:
        let processEndpoint = 
            {hostname, port,...}:{
                ${hostname} = map (env: {inherit env port;}) (builtins.attrValues deployements);
            };
        in {
                tcp = (utils.mergeAll (map processEndpoint endpoints.tcp));
                http = (utils.mergeAll (map processEndpoint endpoints.http));
                udp = (utils.mergeAll (map processEndpoint endpoints.udp));
        };


in {
    imports = [./options
               ./users.nix
               ./assets.nix
               ./links.nix 
               ./storage.nix
               ./endpoints.nix
               ];
    naps.deploy.systems = utils.mergeAll (lib.mapAttrsToList processVM config.naps.topology.vms);
    naps.deploy.endpoints = utils.mergeAll (lib.mapAttrsToList processService config.naps.services);
}
