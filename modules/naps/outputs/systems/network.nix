...
let
    ...
    mkHosts =
        ...
        let proxyAddr = if deploy.env.type == "vm" then "127.0.0.1" else "192.168.100.1";
            caenvs = config.naps.deploy.endpoints."ca.${domain}";
        in 
        ...
        in {
            ...
            ${config.naps.deploy.systems.${cauid}.ip} = ["ca.${domain}"];
        };
    */
    mkHosts =
        deploy:
...
in {
    naps.outputs.systems = utils.mergeAll (lib.mapAttrsToList generateNetworking config.naps.deploy.systems);
}
